...
var passwordHintCenter = {}; //List of password hints
...
function arborescence(folders, files, parent, fullpath) {
	if (folders !== null) {
		for (var i = 0; i < folders.length; i++) {
			...
			var passwordHint = (obj.passwordHint == undefined) ? "" : obj.passwordHint;
			var seqNumber = (obj.password == undefined) ? "" : obj.sequence;
			cFolder(obj.foldername, parent, password, passwordHint, seqNumber);
...
/*folder routine HTML*/
function cFolder(name, parent, password, passwordHint, seqNumber) {
	passwordCenter[name] = password;
	passwordHintCenter[name] = passwordHint;
	if (password != null) {
...
function win() {
	...
	if (finalStepAdded == "none") {
		setTimeout(function () {
			closeNewContact('newContact-window');
		}, 1);
